%% DEMO: main functions
%  In this demo we will present the main functions for normal usage
...
%  We use gettest to calculate the wavenumber k of the solution that we got,
%  and display the value 2*pi/k, that is, the size of a single period
disp(2*pi/gettest(out2,Ps,Es,'Es.TestFunc',@T_CalcWL))
...
[bestst,bestprm] = runfind(Vs,Ps,Es,'Es.FindVal',0.2,'Es.BfPrm','f','Es.FindFunc',@runflow,'Es.FuncList',{@runsim,@T_AvgVal});
%  note that the function we're optimizing is runflow, where we arbitarily running a simulation 
%  for a predefined amount of time, and calculate the average value of variable 1.

%  calculate the average at the original parameters, just for future reference
orgval = gettest(out1,Ps,Es,'Es.TestFunc',@T_AvgVal);
...
subplot(1,2,2);
plotst(bestst,Ps,Es);
title(sprintf('Found point f=%.3f, with avg=%.3f',bestprm(1),bestprm(2)));
