...
#echo "we're on the cluster?"
		matloc="/fastspace/matlab/bin/matlab"  # For cluster computer
        rdmloc="~/fastspace/RDM"
	else
...
echo matlab is in: $matloc
pathcom="tmppath=genpath('$rdmloc'); addpath(tmppath); clear tmppath;"
matoptions="-nojvm -nodesktop -nodisplay -nosplash"
...
if [ "$#" -lt 5 ]; then
	...
	# File collection mode
        printf "attempting to load & collect files of: $1.\n"
        $matloc $matoptions << M_PROG
        $pathcom
        CollectFromCluster('$1');
M_PROG
...
EOJ
`
            echo "running JobID = ${JOBNAME} at queue $queue, part $i, on `date`"
            sleep $sleeptime    # sleeping a few seconds, so things don't overload
	    fi
	else
	    #if [ $queue -eq 0 ]; then
