function VsOut=ILDPS(Vs,Ps,Es,varargin)
% Integrator - Linear Diffusion - Pseudo-Spectral. Based on code by Aly-Khan Kassam (Solving reaction-diffusion equations 10 times faster)
% VsOut=ILDPS(Vs,Ps,Es)
...
for ii=1:Vnum
	...
	Ls{ii}=Ls{ii} - Ds(Vnum+ii)*(eta.^2+xi.^2);		%2D Laplacian. {-D*(eta.^2+xi.^2+zeta.^2)}
	Ls{ii}=Ls{ii} - Ds(Vnum*2+ii)*i*xi.*(eta.^2+xi.^2);	%2D Laplacian*Grad(x)
	Ls{ii}=Ls{ii} + Ds(Vnum*3+ii)*(eta.^2+xi.^2).^2;	%2D Laplacian^2
end;
...
for n = 1:nmax
	...
	for ii=1:Vnum
		...
		Bs{ii}=EE1s{ii}.*Us{ii} + Qs{ii}.*Nvs{ii};	%Coefficient ’b’ in ETDRK formula
